          ...
          foreach ($message->result_array() as $not) {
            ...
            <div class="media text-muted pt-3">
                ...
                <div class="pull-right">
                    <a href="<?php echo(base_url()) ?>admin/chat_admin/<?php echo($id) ?>/<?php echo($not['id_user']) ?>" class="btn btn-default btn-sm"><i class="fa fa-comment"></i>chat</a>

                    <a onclick="return confirm('Etes-vous sûre de vouloire Supprimer ce message?')" href="<?php echo(base_url()) ?>admin/view_1/display_delete_message/<?php echo($id) ?>/<?php echo($not['idmessage']) ?>" class="btn btn-default btn-sm"><i class="fa fa-trash"></i> supprimer</a> 
               </div>
            </div>
